...
From the summary of the baseline
...
model on the different training datasets. 
- The rolling average trained
models performed worse in terms of accuracy and sensitivity and performed slightly better in specificity. This gives some evidence that in order
...
the data from the previous five games. 
...
model performed extremely closely with the simple logit model, usually within 1% of all performance metrics.This is
not an unexpected result as the two models can be used
interchangeably.

#### Decision Trees

<might have to change to 1000 trees of other grid search stuff.>
...
-   <Fact Check this> Overall, the
    ...
    data. The largest difference being in sensitivity of the models while varying the type of averaging of the data.
-   <Fact Check this> The decision tree gave very similar results to the
    logit and probit models when trained in the same way; specificities were in the mid 80 percents while accuracy hovered somewhere around 70 percent.
