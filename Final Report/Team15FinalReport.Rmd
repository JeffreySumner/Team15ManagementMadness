...
would therefore contribute to our model's lack of performance. For
instance, models trained with features with centers close to zero would
sometimes predict only one class of outcome (all wins or all loses).
Removing these types of features brought our features down from 286 to
237 features remaining.
...
The last feature we created was distance traveled to the game site. If
the team was playing a home game, their distance traveled would be zero.
To create this feature, we collected data on game site longitude and
latitude. The distance calculated was the straight-line \<(ellipsoid)\>
distance through the \<`geosphere()`\> function.
