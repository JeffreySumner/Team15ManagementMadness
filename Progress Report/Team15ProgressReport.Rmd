...
-   [4] Pomeroy, Ken. "The Possession" 19 Mar. 2004, 
    <https://kenpom.com/blog/the-possession/#:\~:text=The%20most%20common%20formula%20for,and%20FTA%20%3D%20free%20throw%20attempts>
...
-   What type of models do you plan to use?
    ...
    - (Stretch goal for this Saturday) Simple regression with a few factors for now, performance vs. ESPN (currently)
...
-   How has your data cleaning progressed?
    ...
    -   How we got our bulk data: (data_gathering_ncaahoopR.R)
        -   The collection of data started with using the data sets within the `ncaahoopR` library and combining the datasets that were interesting to our project. Overall, the script `data_gathering_ncaahoopR.R` aggregates box score, play-by-play, rosters and schedule files from the `ncaahoopR` library. The resulting data frame is the basis for ammending and creating all other features. From here on out, the feature `game_id` will be used as the index for which we join new data into the larger data frame.
    ...
    -   How we got/added the Attendance Data: (data_cleaning_attendance.R, data_cleaning_hoopR_attendance.R)
        - At this point, each game in the dataset has a unique identifier called `game_id`. This identifier was used to query the ESPN API to get data on the attendance of the game from https://site.api.espn.com. Another version of this process was also attempted using the `hoopR` package, which had data in its repository files. The returned data was then parsed into a data frame. Notable packages used during this process were: `devtools`, `ncaahoopR`, `tidyverse`, `hoopR`, `tictoc`, `httr`, and `jsonlite`.
        - When data collection / scraping became large and time-consuming, multiple cores were used to make the process more time-efficient.
    -   How we got betting data: (data_cleaning_hoopR_betting.R)
        - Similar to how we got attendance data, we employed the `hoopR` package upon the `game_ids` from the box score dataset. The `espn_mbb_betting(<game_id>)` function allowed retrieval of the relevant data (win probability, spread data, game-projections data) per game from ESPN. Due to the size of the data files, this data had to be stored within 3 different files.
    -   Processing the data into one data frame (data_cleanup.R)
        - After collecting box score, attendance, AP Polls and college location data through previously discussed scripts, the data was processed and merged into one data frame. Some of the processing included: changing variable names and layout within the data frame, estimating positions, calculating defensive and offensive ratings, removing redundant variables, etc.

Summary:
This passage describes the process of collecting and cleaning data for a project related to NCAA basketball. The project involved gathering data on game outcomes, betting odds, attendance, and AP poll rankings for college basketball games between 2012 and 2022. The data was collected using a combination of web scraping and querying APIs.
...
To collect data on AP poll rankings, the author wrote a web-scraping function that collected data from sports-reference.com using the rvest package. The data was then processed using the tidyverse package and lapply() function to combine scraped data frames into a single table.

For attendance data, the author used the game_id identifier to query the ESPN API and retrieve data on attendance. They also attempted to use the hoopR package, which had data in its repository files, but ultimately used the ESPN API.

Similarly, the author used the hoopR package to retrieve betting data for each game using the espn_mbb_betting(<game_id>) function. Due to the size of the data files, this data had to be stored within three different files.

Finally, the author processed and merged all of the collected data into one data frame using the data_cleanup.R script. The script included processing steps such as filtering and renaming variables, geocoding college locations, and merging data frames.
