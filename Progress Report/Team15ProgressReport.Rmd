...
-   How has your data cleaning progressed?
    ...
    -   How we got the AP Polls data: (data_cleaning_ap_polls.R)
        ...
        -   As work progressed, Jeffrey found it easier to acquire the
            AP Polls data via writing a web-scraping function that
            collects the data from sports-reference.com.To do this, the
            `rvest` package was used which has a `read-html` function
            which helped extract the data using `html-nodes` and
            `html_table` functions. The `tidyverse` package and
            `lapply()` function was also used in this process in order
            to mutate and combine scraped data frames into a single
            table. The resulting table contains all the AP Poll data for
